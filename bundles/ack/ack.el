...
(defvar ack-command "/Users/alexp/bin/ack --nocolor --nogroup "
...
(defun ack (command-args)
  (interactive
   (list (read-from-minibuffer "Run ack (like this): "
                               ...
                               nil
                               'ack-history)))
  (compilation-start (concat command-args " " default-directory) 
                     'ack-mode))

;;; Ack in the root of Rails/merb project
;;; Uses function app-root provided by rspec-mode.el
(defun ack-project (command-args)
  (interactive
   (list (read-from-minibuffer "Run ack in project (like this): "
                               (ack-build-command)
                               nil
                               nil
                               'ack-history)))
  (compilation-start (concat command-args " " (app-root)) 
                     'ack-mode))
